...
The landing app is a configuration-driven index for projects in this monorepo and explicitly registered adjacent GitHub repositories. Project data, site copy, derived data, and presentation are deliberately separated so that routine catalog updates do not require component changes.
...
│   ├── validateCatalog.ts     # Sources, taxonomy, paths, covers, and asset-copy checks
...
`projectCatalog` is the source of truth. The page derives section lists, the latest-project carousel, workbench groups, repository links, and header counts from it. Validation scans the established `apps`, `packages`, `workbench`, and `others` taxonomy so a new project directory cannot be omitted accidentally; only the landing index itself is excluded. Explicit external repositories remain outside that filesystem coverage. Duplicate source identities or visible titles and invalid update dates fail with a clear error.

Every project renders in exactly one section. A project with `showcase` appears in Latest projects and is excluded from its catalog or workbench section, preventing the same card from being repeated elsewhere on the page. Every rendered collection places `archived` entries after active or lifecycle-free entries. Each lifecycle group is then ordered by `updatedAt` from newest to oldest, with the title as a deterministic tie-breaker. Catalog card numbers are generated from that final order, so adding or updating a project never requires renumbering the manifest. Workbench categories retain the order declared in `src/config/projects/workbench.ts`, while the projects inside each category follow the shared update order.

`pnpm validate` checks catalog source identities and taxonomy, repository paths, public covers, declared PNG dimensions, canonical asset copies, site and SEO configuration, exclusive derived collection membership and ordering, stylesheet ownership, Vercel deployment configuration, the static document shell, structured data, crawl assets, and server-rendered markup relationships. It runs automatically before `dev` and as part of this app's existing `check` and `build` commands; no separate CI workflow is required. The build then runs `validateBuild.ts` against `dist/` so an empty app shell, unresolved SEO token, or stale robots and sitemap output cannot ship.
...
The Open bench keeps a low-contrast static mosaic beneath a fixed enhancement layer. Three offset fields of muted color float vertically around the heading beneath the layer's tiled mask, giving the nearby mosaic changing depth without creating a circular spotlight or shifting the tile grid itself. The CSS-only motion reverses smoothly without following the pointer, and reduced-motion preferences keep it fully static.
...
Every entry needs a unique source identity, `title`, `summary`, `kind`, and ISO `updatedAt`. Use a repository-relative `path` for projects owned by Cedarflake Lab. A catalog entry for an adjacent GitHub repository uses `repositoryUrl` instead; it must be the canonical `https://github.com/<owner>/<repository>` root without `.git`, a trailing slash, query, fragment, or nested tree path. Declare exactly one of `path` and `repositoryUrl`. The source identity is also the stable React key, while visible card numbers are derived automatically after sorting. Catalog entries declare a visible format `label` and an explicit `lifecycle` of `active` or `archived`; archived styling and the `Archived` badge are both derived from that lifecycle instead of separate presentation flags. Text values must not contain surrounding whitespace. Internal paths use portable forward slashes without empty, `.` or `..` segments; URL-sensitive characters inside a segment are encoded automatically.

Every project card retains a whole-card primary link to its GitHub Source URL, including projects that also have a Live site or Install channel. Internal entries derive that URL from `path`; external entries use their canonical `repositoryUrl`. The footer also renders a compact icon-only Source link. An optional `externalAction` adds exactly one compact icon-only link before Source; every icon link has an explicit accessible name. The primary and footer links remain independent, non-nested links so each icon can be focused and activated separately without changing the card destination. Use `kind: "live"` only for a verified canonical deployment synchronized with the owning repository README and project-owned web metadata; use `kind: "install"` only for an externally verified distribution synchronized with the project README and its owning registry or generated userscript metadata. An Install destination must identify the real installation channel rather than a source or documentation page. Workbench entries remain source-only under the current local-first presentation.

For repository-relative entries, monorepo validation requires an app Live action to have one non-empty document title and description, rejects known scaffold metadata, and requires language, a deployable favicon, a canonical URL, matching Open Graph and Twitter fields, and an explicit `robots.txt`; raw-main userscript Install actions must target a generated `.user.js` inside the owning project and match its `@downloadURL` and `@updateURL`. An external repository is not read from the Cedarflake Lab checkout, so its README, deployment, and metadata alignment must be verified against that repository before recording a Live action.

For repository-relative entries, keep the path taxonomy consistent with the entry: `app` uses `apps/`, `package` uses `packages/`, `workbench` uses `workbench/`, and `other` uses `others/`. The primary section must match that root, and a workbench project's category must match the second path segment. External repository entries currently use the compact catalog presentation and retain their product `kind` for aggregate counts. Validation rejects mismatches before they can produce incorrect groups or counts.

Add a `showcase` object when a project should appear in the latest-project carousel. A showcase needs a label, tags, and a real cover image with alt text and intrinsic dimensions. Any repository-relative primary presentation and any explicitly registered external catalog project can opt in, and the carousel includes every opted-in project without a fixed card limit. Opting in promotes the project into Latest projects and removes it from its catalog or workbench section, so no project card appears twice. The carousel follows the shared lifecycle-first, then newest-first ordering. For an external repository, Landing owns the deployed cover copy; capture it from the project's actual UI or output and verify it against the owning repository before publishing it.
...
Edit [`src/config/site.ts`](./src/config/site.ts) for the locale, time zone, navigation, headings, hero copy and artwork metadata, Open bench workspace-area copy, and footer text. Project content should remain in the project manifest. The document language, project-date formatter, and deterministic title sorting all consume the shared locale configuration. Declared hero artwork dimensions are validated against the deployed PNG and drive its layout ratio. The Cedarflake Lab repository URL must be a canonical `https://github.com/<owner>/<repository>` URL with a portable Git branch name because internal catalog entries derive GitHub tree URLs from them.
