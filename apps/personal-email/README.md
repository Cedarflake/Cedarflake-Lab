...
- `scripts/renderEmail.tsx`：将模板渲染到 `dist/output`，支持通过 CLI 或环境变量选择模板与参数
...
- 生成指定模板：
```bash
pnpm run render:email -- welcome
pnpm run render:email -- reset_password 李四
pnpm run render:email -- notification 王五
```
...
- 渲染结果默认写入 `dist/output/<template>.html`；如需同时打印完整 HTML，可设置 `PRINT_HTML=1`。
- 邮件模板尽量使用内联样式以提升各种客户端的兼容性。

如何集中修改邮件内容

- 在 `src/config/company/emailContent.ts` 或 `src/config/personal/emailContent.ts` 中维护 `templates` 对象，添加或编辑模板。
