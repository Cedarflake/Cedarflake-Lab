...
本仓库包含**代码**与**非代码素材**两类内容，其许可范围分别如下：

1. **代码与工程文件**
	- 除另有说明外，本项目中由仓库作者提供的源代码、脚本、配置文件与文档，均依据 [BSD 3-Clause License](./LICENSE) 开源发布。
	- 你可以在遵守该许可证条款的前提下，对上述内容进行使用、修改与再分发。

2. **艺术设计、游戏素材与相关标识**
	- 本项目中涉及的游戏名称、商标、角色形象、界面风格、图片素材及其他相关美术资源，其著作权、商标权及其他权利归各自权利人所有；其中与 **maimai** 相关的艺术设计及素材权利归 **SEGA** 或相关权利方所有。
	- 上述非代码内容**不因本仓库开源而自动授予任何额外许可**；本仓库的开源许可仅适用于可由仓库作者授权的代码与工程文件部分。

3. **第三方依赖**
	- 本项目所使用的第三方库、字体及其他依赖，分别适用其各自的许可证；具体条款请以对应项目官方说明为准。

4. **使用说明**
	- 如你计划将本项目中的相关素材用于公开发布、商业用途或二次创作，请自行确认是否已取得必要授权。
	- 本项目仅为技术研究、工程化实现与效果演示之用，与 **SEGA** 及相关权利方不存在官方关联。